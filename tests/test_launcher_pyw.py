"""Tests for the silent Windows launcher (``MarkdownAll.pyw``)."""

from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
PYW = ROOT / "MarkdownAll.pyw"


def _load():
    import importlib.util

    spec = importlib.util.spec_from_file_location("markdownall_launcher", PYW)
...
def test_pyw_is_stdlib_only_and_self_contained():
    ...
    for banned in ("PySide6", "markdownall.ui", "import markdownall"):
        assert banned not in text


def test_pyw_delegates_to_module():
    text = PYW.read_text(encoding="utf-8")
    assert '_MODULE = "markdownall.launch"' in text
...
def test_windowed_python_prefers_base_python_exe(tmp_path, monkeypatch):
    ...
    # Base python.exe: CREATE_NO_WINDOW gives an invisible console that child
    # console tools inherit, and it is not the uv shim.
...
def test_windowed_python_falls_back_to_venv_launcher(tmp_path, monkeypatch):
    ...
    # no pyvenv.cfg -> fall back to the venv launcher
...
def test_relaunch_targets_the_pyw_and_activates_the_venv(tmp_path, monkeypatch):
    ...
    args, kwargs = calls[0]
    # Launch the *base* interpreter on this .pyw (not `-m ...` via shim).
