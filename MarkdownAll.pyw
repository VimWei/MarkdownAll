"""Double-click launcher for MarkdownAll.
...
* uses ``python.exe`` (not ``pythonw.exe``) together with ``CREATE_NO_WINDOW``,
  which allocates an *invisible* console. MarkdownAll shells out to console
  tools (``uv version --short``, ...); a console-less ``pythonw`` parent would
  make each child pop its own console window;
* activates the venv via ``__PYVENV_LAUNCHER__`` and points the child's
  stdout/stderr at ``%LOCALAPPDATA%\\MarkdownAll\\launcher.log``.

The application entry point is unchanged, so the start-up splash screen
(created inside ``markdownall.launch.main`` before the heavy imports) is fully
preserved. The visible entry point for troubleshooting is:
...
def _log_path() -> Path:
    ...
    return Path(base) / "MarkdownAll" / "launcher.log"
